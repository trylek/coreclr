...
set __CrossgenOut="Crossgen-ret.out"
...
if defined __DoCrossgen2 ( set __CrossgenExe="!CORE_ROOT_STAGE!\crossgen2\crossgen2.exe";set __CrossgenOut="CPAOT-ret.out" )
echo Use Crossgen executable: %__CrossgenExe%
...
set FrameworkCmd=%FrameworkCmd% -cp %__CrossgenExe%
set FrameworkCmd=%FrameworkCmd% --release
set FrameworkCmd=%FrameworkCmd% --large-bubble
set FrameworkCmd=%FrameworkCmd% %*
echo Compiling framework: %FrameworkCmd%
%FrameworkCmd%
set FrameworkExitCode=%ERRORLEVEL%
if "%FrameworkExitCode%" == "0" (
  echo Successfully precompiled framework
  copy /Y %CORE_ROOT%\%__CrossgenOut%\*.dll %CORE_ROOT%\
  exit /b 0
) else (
  echo Failed precompiling framework
  exit /b %FrameworkExitCode%
)
